...
from ...experimental.experimental_utils.experimental_terrains import QUADCOPTER_ROUGH_TERRAINS_CFG_FACTORY

class LidarObstacleAvoidanceEnvCfgWindow(BaseEnvWindow):
    """Window manager for the Quadcopter environment."""

    def __init__(self, env: LidarObstacleAvoidanceEnvCfg, window_name: str = "IsaacLab"):
...
@configclass
class LidarObstacleAvoidanceEnvCfg(DirectRLEnvCfg):
    # env
    ...
    ui_window_class_type = LidarObstacleAvoidanceEnvCfgWindow
...
class LidarObstacleAvoidanceEnv(DirectRLEnv):
    cfg: LidarObstacleAvoidanceEnvCfg

    def __init__(self, cfg: LidarObstacleAvoidanceEnvCfg, render_mode: str | None = None, **kwargs):
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
        ...
        self._thrust[:, 0, 2] = self.cfg.thrust_to_weight * self._robot_weight * (
                    self.cfg.thrust_scale * self._actions[:, 0] + 1.0) / 2.0
    ...
    def _get_observations(self) -> dict:
        ...
        root_state_w = self._robot.data.root_state_w.unsqueeze(1).repeat(1, self._simple_lidar.data.ray_hits_w.shape[1],
                                                                         1)
    ...
    def _sample_points_square_hole(self, n: int, side_length: float, hole_length: float):
        ...
        points[idx_splits[2]] = torch.stack([
            ...
            # x-coordinates (left of the hole)
        ...
        points[idx_splits[3]] = torch.stack([
            ...
            # x-coordinates (right of the hole)
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        self._desired_pos_w[env_ids, :2] = self._sample_points_square_hole(len(env_ids),
                                                                           2 * self.cfg.desired_pos_b_xy_limits[1],
                                                                           2 * self.cfg.desired_pos_b_xy_limits[0])
...
class LidarObstacleAvoidanceEnvCfg_PLAY(LidarObstacleAvoidanceEnvCfg):
